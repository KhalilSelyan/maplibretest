...
export function parseChannel(channel: Channel): ParsedChannel {
  if (channel.messageEncoding === "json") {
    if (
      ...
      channel.schema.encoding !== "jsonschema"
    ) {
      throw new Error(
    ...
    let deserialize = (data: ArrayBufferView) =>
    ...
    if (channel.schema != undefined) {
      ...
      if (schema != undefined) {
        if (typeof schema !== "object") {
          throw new Error(
            `Invalid schema, expected JSON object, got ${typeof schema}`
          );
        }
        const { datatypes: parsedDatatypes, postprocessValue } =
          parseJsonSchema(
        ...
        deserialize = (data) =>
          postprocessValue(
            JSON.parse(textDecoder.decode(data)) as Record<string, unknown>
          );
      }
    }
  ...
  if (channel.messageEncoding === "ros1") {
    ...
    return {
      datatypes: parsedDefinitionsToDatatypes(
        parsedDefinitions,
        channel.schema.name
      ),
