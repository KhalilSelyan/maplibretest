...
import { type MessageDefinitionMap } from "./parseChannel";
import { type MessageDefinitionField } from "./def";
...
export function parseJsonSchema(
  ...
  function addFieldsRecursive(
    ...
    let postprocessObject: (value: Record<string, unknown>) => unknown = (
      value
    ) => value;
    const fields: MessageDefinitionField[] = [];
    if (schema.type !== "object") {
      throw new Error(
        `Expected "type": "object" for schema ${typeName}, got ${JSON.stringify(
          schema.type
        )}`
      );
    }
    for (const [fieldName, fieldSchema] of Object.entries(
      ...
      if (Array.isArray(fieldSchema.oneOf)) {
        if (
          fieldSchema.oneOf.every(
            (alternative) => typeof alternative.const === "number"
          )
        ) {
              ...
              postprocessObject = (value) => {
                ...
                if (typeof str === "string") {
                  ...
                  if (base64.decode(str, decoded, 0) !== decoded.byteLength) {
                    throw new Error(
                      `Failed to decode base64 data for ${keyPath
                        .concat(fieldName)
                        .join(".")}`
                    );
                  }
        ...
        case "object": {
          ...
          postprocessObject = (value) => {
            ...
            if (fieldValue != undefined && typeof fieldValue === "object") {
              value[fieldName] = postprocessNestedObject(
